...
def parse_contents():
    ...
    for i, match in enumerate(chapter_matches):
        ...
        chapters.append(
            {"number": chapter_num, "title": chapter_title, "volume": volume, "files": files}
        )
...
def parse_line_content(line):
    ...
    return {"line": line_num, "tag": tag, "text": text}
...
def merge_layers(tibetan_files, literal_files, liturgical_files):
    ...
    all_lines = sorted(
        set(list(tibetan_data.keys()) + list(literal_data.keys()) + list(liturgical_data.keys())),
        key=lambda x: int(x),
    )
...
def generate_review_prompt(chapter_num, chapter_title, merged_data):
    ...
    # Header
    lines.append(
        "Conduct deep scan and critical honest review of the attached Tibetan Dzogchen translation in light of the literal and liturgical mission statements."
    )
    lines.append("")
    lines.append(
        "If you have suggestions for specific lines return them in code block in the strict line format:"
    )
    lines.append("")
    ...
    lines.append(
        "[725]<liturgical><prose>The antidote for the three poisons of desire, anger, and delusion is the realization of emptiness."
    )
    lines.append("")
    lines.append("=" * 60)
    lines.append("")
