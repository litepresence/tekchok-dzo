...
def parse_entry(lines):
    ...
    """
    entry = {"line_range": None, "tag": None, "heading": None, "sections": []}
...
def build_content_text(entry):
    ...
    for section in entry["sections"]:
        if section["heading"]:
            html_parts.append(
                f'<div class="section-heading">{html.escape(section["heading"])}</div>'
            )
        ...
        content_text = re.sub(
            r"\*\*Analysis:\*\*", '<span class="analysis-header">Analysis:</span>', content_text
        )
        content_text = re.sub(
            r"\*\*Section Placement:\*\*",
            '<span class="analysis-header">Section Placement:</span>',
            content_text,
        )
        content_text = re.sub(
            r"\*\*Connection to Following Sections:\*\*",
            '<span class="analysis-header">Connection to Following Sections:</span>',
            content_text,
        )
        content_text = re.sub(
            r"\*\*Terminological Precision:\*\*",
            '<span class="analysis-header">Terminological Precision:</span>',
            content_text,
        )
        content_text = re.sub(
            r"\*\*Key Terms:\*\*", '<span class="analysis-header">Key Terms:</span>', content_text
        )
...
def generate_file_html(parsed_lines, filename):
    ...
    for entry in parsed_lines:
        ...
        html_parts.append(
            f'<div class="entry-block {tag_class}" id="line-{unique_id}" data-line="{first_line}" data-range-start="{first_line}" data-range-end="{last_line}">'
        )
        ...
        if entry["tag"]:
            ...
            html_parts.append(
                f'    <span class="tag tag-{entry["tag"]}">{entry["tag"].title()}</span>'
            )
...
<!-- Edit /js/scholar.js and run process_scholar.py to regenerate. -->
...
<head>
    ...
    <!-- Edit the source files and run process_scholar.py to regenerate. -->
    <meta charset="UTF-8">
