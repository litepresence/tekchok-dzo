...
OUTPUT_FILE = Path("literal_proof.html")
...
<style>
    ...
    * {
    ...
    /* Line grid for literal translations */
    .line-grid {
        display: grid;
    ...
    /* Hyphenated compounds - keep together */
    .compound {
        color: #333;
        white-space: nowrap;
    }

    /* Grammatical markers (genitive, etc.) - keep attached to previous word */
    .grammar-marker {
        font-variant: small-caps;
        font-size: 0.85em;
        color: var(--color-accent);
...
def process_content(content):
    ...
    for marker in grammar_markers:
        ...
        pattern = rf'(-{marker})(\s|$)'
        replacement = r'<span class="grammar-marker">\1</span>\2'
...
def generate_file_html(parsed_lines, filename):
    ...
    for item in parsed_lines:
        ...
        html_parts.append(f'  <div class="line-content"><span class="literal-line" data-line="{line_num}">{content}</span></div>')
...
def build_full_document(all_file_content, toc_entries):
    ...
    # Build TOC
    toc_html = '<div class="toc"><h1>Literal Translation Index</h1><ul>'
