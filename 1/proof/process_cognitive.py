...
def parse_entry(lines):
    ...
    entry["content_text"] = re.sub(
        r"/([\w\W]+?)/", r'<span class="term">/\1/</span>', entry["content_text"]
    )

    # Mark IAST transliteration (text in +plus+)
    entry["content_text"] = re.sub(
        r"\+([^+]+?)\+", r'<span class="iast">\1</span>', entry["content_text"]
    )
...
<!-- Edit /js/cognitive.js and run process_cognitive.py to regenerate. -->
...
<head>
    ...
    <!-- Edit the source files and run process_cognitive.py to regenerate. -->
    <meta charset="UTF-8">
