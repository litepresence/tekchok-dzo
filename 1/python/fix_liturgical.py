...
def extract_parts(line):
    # Format: "[12500] content" or "[12500]།content"
    match = re.match(r'^(\[\d+\])\s*[།/]?\s*(.*)$', line)
    if match:
        bracket = match.group(1)  # [12500]
        content = match.group(2).strip()
        # Extract number from bracket
        num_match = re.search(r'\d+', bracket)
        bracket_num = num_match.group(0) if num_match else ""
        return bracket, bracket_num, content
    return None, None, line.strip()

# Comprehensive liturgical translations
translations = {
...
for page in range(301, 324):
    ...
    for line in lines:
        bracket, bracket_num, content = extract_parts(line)
        if bracket and bracket_num.isdigit() and int(bracket_num) in translations:
            trans = translations[int(bracket_num)]
