...
Cleans duplicate line numbers from source (e.g., '[163] [163]' -> '[163]').
"""
...
def read_page_lines(page_path):
    """Read a PAGE file and return list of (line_number, content) tuples.
    Cleans duplicate line numbers like '[163] [163] content' -> 'content'."""
    lines = []
    ...
    with open(page_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if match:
                ...
                # Check if content starts with duplicate [number]
                dup_match = re.match(r'^(\[\d+\])\s*(.*)', content)
                if dup_match:
                    # Remove duplicate line number from content
                    content = dup_match.group(2)
