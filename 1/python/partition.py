...
with open("boundary_v2.json", "r", encoding="utf-8") as f:
    data = json.load(f)
...
# print("\033c")
# print("\n".join([", ".join(str(j) for j in i.values()) for i in sections]))
# exit()


sections = []
for section_name, section_data in data["sections"].items():
    vol, chap, sec, sub = map(int, section_name.split("-"))
    if vol != volume:
        continue
        ...
        dict(
            zip(
                ...
                [
                    ...
                    chap,
                    f"{sec}_{sub}",
                    section_data["start_page"],
                    section_data["start_line"],
                    section_data["start_phrase"],
                ],
            )
        )
    )
...
for type_idx, kind in enumerate(page_data):
    ...
    for line in kind:
        ...
        if pdx >= len(pairs):
            pdx -= 1
        if pairs[pdx][0]["start_line"] == line_num:
            ...
            if not type_idx:
                if pairs[pdx][0]["split_phrase"] not in line:
                    print(line, pairs[pdx][0])

                splittage = line.split(pairs[pdx][0]["split_phrase"])
                if len(splittage) > 1 and splittage[0]:
                    buf[-1][-2].append((line_num, splittage[0]))
                after = pairs[pdx][0]["split_phrase"] + splittage[-1]
