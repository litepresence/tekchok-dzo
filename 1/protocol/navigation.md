...
├── contents/                    # Structural definitions
│   ├── boundary.json           # Master structural boundaries (213 sections)
│   ├── boundary_concise.json   # Compact boundary data
│   ├── contents.md             # Human-readable table of contents
│   └── voice_contents.md        # Voice-specific content mappings
├── prompt/                     # Layer-specific prompts and guidance
│   ├── OVERVIEW.md             # Project overview
│   ├── prompt_tibetan.md       # Tibetan source layer guidance
│   ├── prompt_wylie.md         # Wylie transliteration guidance
│   ├── prompt_literal.md       # Literal layer guidance
│   ├── prompt_liturgical.md    # Liturgical layer guidance
│   ├── prompt_commentary.md    # Commentary layer guidance
│   ├── prompt_scholar.md      # Scholar layer guidance
│   ├── prompt_epistemic.md     # Epistemic layer guidance
...
│   ├── markers.md             # Section markers reference
│   ├── capitalize.md          # Capitalization standards
│   ├── glossary.md            # Comprehensive terminology glossary
│   ├── voices.md              # Voice/persona specifications
│   ├── metaphors.md           # Metaphor standards
│   ├── meta.md                # Meta-analysis and insights
...
├── reports/                    # QC and processing reports
├── text/                       # PRIMARY BUILD - Section-based structure
│   ├── frozen/                 # Immutable layers (FINAL - DO NOT MODIFY)
│   │   ├── tibetan/           # TSHAD MA - Clean source text (213 files)
│   │   ├── wylie/            # LAM - Extended Wylie transliteration (213 files)
│   │   ├── literal/           # Dpyad kyi bshad pa - 1:1 grammatical (209 files)
│   │   ├── liturgical/        # sgrub pa'i gleng gzhi - Vajra speech (214 files)
...
**Frozen Layers (Immutable - Final):**
| Layer | Files | Purpose |
|-------|-------|---------|
| tibetan | 213 | Clean source text (TSHAD MA) |
| wylie | 213 | Extended Wylie transliteration (LAM) |
| literal | 209 | 1:1 grammatical mapping |
| liturgical | 214 | Vajra speech (sgrub pa'i gleng gzhi) |
| meter | 213 | Metrical analysis |
| artifacts | 213 | Source with PAGE markers |
| raw | 2 | BDRC source import |
| exposition | 12 | Editorial conventions (layer explanations) |
| corpus | 4 | Volume introductions (8-volume structure) |

**Dynamic Layers (Mutable - Work Here):**
| Layer | Files | Purpose |
|-------|-------|---------|
| commentary | 190 | Heart instruction (ngo sprod) |
| scholar | 213 | Academic analysis |
| epistemic | 182 | View stratification |
| delusion | 213 | Error detection |
| cognitive | 73 | Translator log |
| introduction | 28 | Chapter introductions |

### Layer Subfolder Contents
Each layer folder in `text/frozen/` or `text/dynamic/` contains files following `VV-CC-SS-SS.txt` format:
- **VV:** Volume (01 or 02)
- **CC:** Chapter (01-25)
- **SS:** Section number (01-20+)
- **SS:** Subsection (01, 02, etc.)
...
## 🆕 TIBETAN SOURCE FILES STRUCTURE (Updated 2026-02-14)

### Dual Tibetan Source System

The project maintains **two versions** of the Tibetan source text in `text/frozen/`:

| Directory | Contents | Use Case | Files |
|-----------|----------|----------|-------|
| `text/frozen/tibetan/` | **CLEAN** - No PAGE markers, no blank lines, bracketed line numbers preserved | **PRIMARY SOURCE** for all translation work | 213 |
| `text/frozen/artifacts/` | Original with PAGE markers (`### PAGE X`) and blank lines | Reference/archive only | 213 |
| `text/frozen/raw/` | BDRC source files (UT22920_005_0001/2) | Original BDRC import | 2 |

### File Format Comparison

**text/frozen/tibetan/ (CLEAN - use this for work):**
```
[1] ༄༅།
...
**text/frozen/artifacts/ (with artifacts - reference only):**
```
[1] ༄༅།
[2] །ཡཱནཱགྲརཏྣཀོཥནཱམབིཛཧཱརམྲ།
...
### Always Use text/frozen/tibetan/

All verification scripts, boundary checks, and translation work use `text/frozen/tibetan/` (the clean version):

```bash
# CORRECT - Use clean source:
cat "/home/opencode/MDZOD/1/text/frozen/tibetan/01-01-01-01.txt"
wc -l "/home/opencode/MDZOD/1/text/frozen/tibetan/01-01-01-01.txt"

# Reference only - contains PAGE markers:
# cat "/home/opencode/MDZOD/1/text/frozen/artifacts/01-01-01-01.txt"
```
...
All page files follow: `VV-CC-SS-SS.txt` format (zero-padded)
- Example: `01-01-01-01.txt`, `01-04-12-01.txt`, `02-18-05-01.txt`
...
cat "/home/opencode/MDZOD/1/text/dynamic/commentary/01-01-01-01.txt"
...
**Path Structure:**
- Frozen layers: `text/frozen/[layer]/VV-CC-SS-SS.txt`
- Dynamic layers: `text/dynamic/[layer]/VV-CC-SS-SS.txt`
...
1. Read `protocol/status.md` - Understand current priorities
2. Read `protocol/conventions.md` - Inherit methodology and quality standards
3. Read `protocol/khenpo.md` - Internalize quality standards
4. Read `prompt/prompt_[layer].md` - Layer-specific guidance (e.g., prompt_commentary.md)

### While Working:
5. Follow layer-specific prompts in `prompt/prompt_[layer].md`
6. Update `protocol/status.md` after completing significant work
7. Maintain voice consistency (Vairotsana, Patrul, etc.)
8. Verify line ranges match Tibetan source (`text/frozen/tibetan/`)

### After Completing Work:
9. Document progress in appropriate location
10. Update `protocol/status.md` with statistics
...
- **protocol/status.md** - Project-wide status and metrics
- **protocol/conventions.md** - Methodology and quality standards
- **protocol/exemplars.md** - Best-practice examples (with section mappings)
- **protocol/capitalize.md** - Capitalization standards for Sanskrit/Dzogchen terms
- **protocol/dictionary.md** - Tibetan-English terminology standards
- **protocol/khenpo.md** - Quality review standards
- **protocol/markers.md** - Section markers reference
- **contents/boundary.json** - Master structural data (213 sections)
- **contents/contents.md** - Human-readable documentation
- **prompt/** - Layer-specific prompts (prompt_commentary.md, prompt_scholar.md, etc.)
- **reports/** - QC reports and certification documents
...
```
**Output:** Creates `contents/verified.json` with True/False for each section
**Status:** All 213 sections verified ✅
**Source:** References `text/frozen/tibetan/` for verification
...
**Output:** Creates `contents/title_verified.json`  
...
**Primary Build Location:** All work happens in `text/dynamic/` (mutable) or `text/frozen/` (immutable):
```bash
# CORRECT - Current section-based format:
cat "/home/opencode/MDZOD/1/text/frozen/tibetan/01-01-01-01.txt"
cat "/home/opencode/MDZOD/1/text/dynamic/liturgical/01-04-12-01.txt"
...
ls "/home/opencode/MDZOD/1/text/frozen/tibetan/01-04-12-01.txt"

# 2. Verify it has content
wc -l "/home/opencode/MDZOD/1/text/frozen/tibetan/01-04-12-01.txt"

# 3. Check line numbering
tail -5 "/home/opencode/MDZOD/1/text/frozen/tibetan/01-04-12-01.txt"

# 4. Check meter classification
grep "01-04-12-01" "/home/opencode/MDZOD/1/text/frozen/meter/"*.txt | head -3
```
...
**NEVER modify (frozen layers - FINAL):**
- `text/frozen/tibetan/` (source text - TSHAD MA - clean version)
- `text/frozen/artifacts/` (original source with PAGE markers - reference only)
- `text/frozen/wylie/` (transliteration - LAM)
- `text/frozen/raw/` (BDRC import)
- `text/frozen/literal/` (1:1 grammatical mapping - immutable)
- `text/frozen/liturgical/` (Vajra speech - immutable)
- `text/frozen/meter/` (metrical analysis - immutable)
- `text/frozen/exposition/` (editorial conventions - FINAL)
- `text/frozen/corpus/` (volume introductions - FINAL)

**ONLY modify (dynamic layers):**
- `text/dynamic/commentary/`
- `text/dynamic/scholar/`
- `text/dynamic/epistemic/`
- `text/dynamic/delusion/`
- `text/dynamic/cognitive/`
- `text/dynamic/introduction/`
...
| Layer | Files | Location | Status |
|-------|-------|----------|--------|
| Tibetan | 213 | frozen/tibetan | Immutable |
| Wylie | 213 | frozen/wylie | Immutable |
| Literal | 209 | frozen/literal | Immutable |
| Liturgical | 214 | frozen/liturgical | Immutable |
| Meter | 213 | frozen/meter | Immutable |
| Artifacts | 213 | frozen/artifacts | Reference |
| Raw | 2 | frozen/raw | Import |
| Exposition | 12 | frozen/exposition | Final |
| Corpus | 4 | frozen/corpus | Final |
| Commentary | 190 | dynamic/commentary | Active |
| Scholar | 213 | dynamic/scholar | Active |
| Epistemic | 182 | dynamic/epistemic | Active |
| Delusion | 213 | dynamic/delusion | Active |
| Cognitive | 73 | dynamic/cognitive | Active |
| Introduction | 28 | dynamic/introduction | Active | |

---

**Navigation Guide Version:** 7.0  
**Last Updated:** 2026-02-14  
**Major Changes:** 
- ✅ **EXPOSITION & CORPUS:** Added to frozen layers (exposition: 12, corpus: 4)
- ✅ **NEW PROMPTS:** Added prompt_exposition.md and prompt_corpus.md
- ✅ **UPDATED FILE COUNTS:** Frozen (tibetan:213, wylie:213, literal:209, liturgical:214, meter:213, artifacts:213, raw:2, exposition:12, corpus:4), Dynamic (commentary:190, scholar:213, epistemic:182, delusion:213, cognitive:73, introduction:28)
- ✅ **FIXED INTEGRITY:** Updated preserve source section to reflect correct layer locations
**Project Root:** `/home/opencode/MDZOD/1/`  
**Frozen Layers:** `text/frozen/` (9 subdirs - immutable source + publication)  
**Dynamic Layers:** `text/dynamic/` (6 subdirs - mutable translation)  
**Structural Data:** `contents/boundary.json`  
**Layer Prompts:** `prompt/prompt_[layer].md` (14 prompt files)
